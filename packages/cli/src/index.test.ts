...
describe("cli analyze command", () => {
  ...
  it("sync migrates convert-owned current facades when switching tenant defaults", () => {
    const repoDir = createTempRepo("rn-mt-cli-sync-current-facade-migration-");
    const stdout = vi.fn();
    const stderr = vi.fn();
    const currentHomePath = join(
      repoDir,
      "src",
      "rn-mt",
      "current",
    ...
    const oldCurrentHomeContents = [
      "// Generated by rn-mt. CLI-owned current facade. Do not edit directly.",
      'export { default } from "../../../tenants/acme/app/(tabs)/index";',
      'export * from "../../../tenants/acme/app/(tabs)/index";',
      "",
    ].join("\n");
    const staleCurrentHomeHash = createHash("sha256")
      .update("stale-generated-current-facade")
      .digest("hex");
    ...
    mkdirSync(dirname(currentHomePath), { recursive: true });
    ...
    writeFileSync(
      ...
      JSON.stringify({
        schemaVersion: 1,
        source: { rootDir: repoDir },
        defaults: { tenant: "bravo", environment: "dev" },
        tenants: {
    ...
    writeFileSync(currentHomePath, oldCurrentHomeContents);
    writeFileSync(
    ...
    const ownershipMetadata = JSON.parse(
    ...
    expect(readFileSync(currentHomePath, "utf8")).toContain(
      "../../../tenants/bravo/app/(tabs)/index",
    );
    expect(ownershipMetadata.artifacts).toEqual(
  ...
  it("allows repo commands to proceed when rn-mt was installed from a packed tarball", () => {
    const repoDir = createTempRepo("rn-mt-cli-version-packed-compatible-");
  ...
  it("fails fast with upgrade guidance when global and local rn-mt cli versions drift", () => {
    ...
    expect(parsed.reason).toContain(
      `Global rn-mt CLI version ${getPublicPackageVersion()} is incompatible with repo-local @_molaidrislabs/rn-mt version 0.2.0.`,
    );
    expect(parsed.compatibility).toEqual({
      globalVersion: getPublicPackageVersion(),
      localVersion: "0.2.0",
  ...
  it("upgrades package versions and migrates legacy hook metadata before sync and audit", () => {
    ...
    expect(upgradedPackageJson.dependencies?.["@_molaidrislabs/rn-mt"]).toBe(
  ...
  "compilerOptions": {
    ...
    writeFileSync(
      ...
      ["export function BrandButton() {", '  return "brand";', "}", ""].join(
        "\n",
      ),
    );
    ...
    const rewrittenTsconfig = readFileSync(
      join(repoDir, "tsconfig.json"),
      "utf8",
    );
  ...
  it("renames a tenant end to end and keeps the renamed default tenant syncable", () => {
    ...
    expect(
      existsSync(
        join(repoDir, "ios", "rn-mt.generated.acme-beta-dev.xcconfig"),
      ),
    ).toBe(true);
    expect(
  ...
  it("handoff preflight passes when the repo is converted, doctor-clean, and has reconstruction metadata", () => {
    ...
    expect(
      outputPackageJson.dependencies?.["@_molaidrislabs/rn-mt"],
    ).toBeUndefined();
