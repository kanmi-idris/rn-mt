...
export class RnMtHandoffModule {
  ...
  flatten(options: {
    ...
    const restoredFiles = reconstructionMetadata.entries.map((entry) => {
      ...
      const selectedContents = isFacadeSourceFile(selectedSourcePath)
        ? this.dependencies.workspace.readText(selectedSourcePath)
        : this.dependencies.workspace.readBuffer(selectedSourcePath);
      const rewrittenContents = isFacadeSourceFile(selectedSourcePath)
        ? (() => {
            if (Buffer.isBuffer(selectedContents)) {
              throw new Error(
                `Expected handoff source contents to be text: ${selectedSourcePath}`,
              );
            }

            return rewriteHandoffSourceContents(
  ...
  cleanup(
    ...
    for (const expoConfigPath of [
      join(this.dependencies.workspace.rootDir, "app.config.ts"),
      join(this.dependencies.workspace.rootDir, "app.config.js"),
    ]) {
      if (
        this.dependencies.workspace.isFile(expoConfigPath) &&
        this.dependencies.workspace
          .readText(expoConfigPath)
          .startsWith(expoComputedConfigBridgeBanner)
      ) {
        candidateRemovedPaths.push(expoConfigPath);
      }
    }
