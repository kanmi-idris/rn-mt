...
import { isAuditableTextFile, isTestSourcePath, listSharedFiles } from "../convert";
...
export class RnMtAuditModule {
  ...
  run(manifest: RnMtManifest): RnMtAuditResult {
    ...
    for (const path of listSharedFiles(this.dependencies.workspace)) {
      ...
      if (this.dependencies.workspace.exists(join(tenantRootDir, relativeSharedPath))) {
        continue;
      }

      const evidence = collectOverrideCandidateEvidence(
        manifest,
